...
//var djActProps = ["A我是红黄马甲（道具）", "B红色故事会（道具）", "C新战疫新力量（道具）", "D党旗（合影旗）", "E入党誓词（合影旗）"];
//var djActRadios = ["A我是红黄马甲（道具）", "B红色故事会（道具）", "C新战疫新力量（道具）"];
var djActProps = ["A我是红黄马甲（道具）", "B红色故事会（道具）", "C党旗（合影旗）", "D入党誓词（合影旗）"];
var djActRadios = ["A我是红黄马甲（道具）", "B红色故事会（道具）"];
...
Vue.component('step-2', {
    ...
    data: function () {
        return {
            ...
            canCheck: true,
            actProps: [],
            props: JSON.parse(JSON.stringify(djActProps)),
        };
    },
    created: function () {
    ...
    methods: {
        ...
        changeCheckboxGroup(values) {
           this.canCheck = !values.some(function (v) {
               return djActRadios.indexOf(v) != -1;
           });
        },
        checkboxSubmit() {
            if (!this.actProps.length) {
    ...
    methods: {
        ...
        doClickNext: function (i) {
            this.showMZ(function () {
                ...
                this.mzShow = false;
                _reqSendActGroup1(function (err, data) {
