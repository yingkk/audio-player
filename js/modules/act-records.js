...
var app = new Vue({
    ...
    methods: {
        ...
        reqTabIndex0: function () {
            ...
            _reqGetActAppoed(function (err, items) {
                this.loading = false;
                this.refreshing = false;
                if (err) return;
                this.finished = true;
                this.items = items.map(function (val) {
                    // val.createtime = fgChangeDate(val.createtime);
                    // val.visitDate = fgChangeDate(val.actRelease.startTime).split(' ')[0];
                    val.show = false;
                    return val;
                }.bind(this));
        ...
        cancelGroup: function (item) {
            if (item.setAppo == 3) {
                this.cancelActAppo(item);
            } else if (item.setAppo == 4) {
                this.cancelDJ(item);
            }
        },
        cancelActAppo(item) {
